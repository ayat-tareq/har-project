...
class SinkhornAlign:
    ...
    def __call__(self, A, B):
        K = np.exp(-np.square(np.linalg.norm(A[:, None] - B[None, :], axis=2)) / self.epsilon)
...
class QuantumHungarian:
    def __init__(self, temp=0.2, n_iter=100):
        self.temp = temp
        self.n_iter = n_iter
    ...
    def __call__(self, A, B):
        ...
        time_sim = np.exp(-time_dist / 50.0)
        combined = 0.7 * sim + 0.3 * time_sim
...
def save_plot(fig, name, dataset):
    # Save a matplotlib figure to disk with dataset-specific filename.
    os.makedirs(f"outputs/{dataset}", exist_ok=True)
    fig.savefig(f"outputs/{dataset}/{name}.png")
    plt.close(fig)

def plot_loss(train_losses, test_losses, dataset):
    fig, ax = plt.subplots()
    ax.plot(train_losses, label='Train Loss')
    ax.plot(test_losses, label='Test Loss')
    ax.set_title(f'{dataset} Train vs Test Loss')
    ax.set_xlabel('Epoch')
    ax.set_ylabel('Loss')
    ax.legend()
    save_plot(fig, 'loss_curve', dataset)

def plot_conf_matrix(y_true, y_pred, dataset, classes=None):
    cm = confusion_matrix(y_true, y_pred)
    disp = ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=classes)
    fig, ax = plt.subplots(figsize=(8, 6))
    disp.plot(ax=ax, cmap='viridis', values_format='d')
    plt.title(f'{dataset} Confusion Matrix')
    save_plot(fig, 'confusion_matrix', dataset)
