...
class SimplifiedHybridEncoder(nn.Module):
    def __init__(self, input_dim, latent_dim=128, dropout=0.3, transformer_seq_len=32, num_transformer_layers=1):
        ...
        self.pos_enc = PositionalEncoding(d_model=512, dropout=dropout)
        ...
        self.projector = nn.Sequential(
            nn.Flatten(),
            nn.Linear(512 * self.transformer_seq_len, 2048),
            nn.BatchNorm1d(2048),
            nn.GELU(),
            nn.Dropout(dropout),
...
def fine_tune(encoder, X_train, y_train, X_val, y_val, args):
    ...
    if subset_label_encoder is not None:
        ...
        logger.info(f"Using {num_classes} classes found in train/val sets.")m_classes = len(np.unique(np.concatenate([y_train, y_val])))
...
def main():
    ...
    parser.add_argument("--batch_size", type=int, default=128, help="Batch size"    parser.add_argument("--epochs", type=int, default=1000, help="Epochs for SSL pretraining")
    parser.add_argument("--ft_epochs", type=int, default=1, help="Epochs for fine-tuning    parser.add_argument("--lr", type=float, default=0.0005, help="Learning rate for SSL pre-training")er.add_argument("--ft_lr", type=float, default=0.0005, help="LR for fine-tuning")
    ...
    parser.add_argument("--time_warp_limit", type=float, default=0.4, help="Time warp limit for augmentation")d_argument("--channel_drop_prob", type=float, default=0.2, help="Channel dropout probability")
