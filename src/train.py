...
def linear_probe(X_train, y_train, X_val, y_val, X_test, y_test, encoder, args):
    ...
    if y_pred_ensemble is not None and isinstance(y_pred_ensemble, np.ndarray) and len(y_pred_ensemble) == len(y_test):
        ...
        try:
            ...
            plt.figure(figsize=(max(6, num_present_classes_test*0.8), max(5, num_present_classes_test*0.7)))
    ...
    if Z_test.shape[0] > 1:
        try:
            ...
            tick_label_map = {tick: label for tick, label in zip(colorbar_ticks, present_target_names_tsne)}
...
def main():
    ...
    if args.subset_fraction < 1.0:
        ...
        y_train = subset_label_encoder.fit_transform(y_train)
        y_val = subset_label_encoder.fit_transform(y_val)
        y_test = subset_label_encoder.fit_transform(y_test)
